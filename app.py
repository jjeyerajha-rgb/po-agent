...
def _load_streamlit_secrets() -> None:
    """Load Streamlit Cloud secrets into os.environ for deployment."""
    try:
        for key in (
            "OPENAI_API_KEY",
            "OPENAI_MODEL",
...
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="PO Agent JJ - Product Owner Assistant",
    page_icon="🎯",
    layout="wide",
...
# ---------------------------------------------------------------------------
with st.sidebar:
    st.markdown("## 🎯 PO Agent JJ")
    st.caption("Your AI Product Owner Assistant")
    st.divider()

    if os.getenv("OPENAI_API_KEY"):
        st.session_state.api_key_set = True
        st.success("API key loaded (Streamlit secrets or environment)")
